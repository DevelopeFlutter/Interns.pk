...
Future signup(
  ...
  try {
    var Data =
        "Name=$firstname&Email=$email&Password=$password&last_name=$lastName&birth_date=$birthDate&gender=$gender&mobile_Number=$mobNumber&address=$address&country=$country&city=$city";
    print(Data);
...
Future changePassword(
  ...
  try {
    ...
    var response =
        await API().changePassword(ApiManager.CHANGEPASSWORD, id, data);
    print(data);
    if (response.statusCode == 200) {
      error = false;
      content = jsonDecode(response.body);
...
Future updatePost(
  ...
  try {
    var data =
        "company_name=$companyName&job_title=$jobTitle&job_location=$jobLocation&description=$description&work_placetype=$workPlacetype&job_type=$jobType";
    var response = await API().upDatePost(ApiManager.UPDATEPOST, id, data);
    print(data);
    if (response.statusCode == 200) {
      error = false;
      content = jsonDecode(response.body);
